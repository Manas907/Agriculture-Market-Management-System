...
class krishi_products(db.Model):
    ...
    crop_year=db.Column(db.String(100))
...
@app.route('/cropsproduct')
def agroproducts():
    # query=db.engine.execute(f"SELECT * FROM `krishi_products`") 
    query=krishi_products.query.all()
    return render_template('cropsproduct.html',query=query)
...
def addproduct():
    if request.method=="POST":
        ...
        products=krishi_products(username=username,email=email,productname=productname,productdesc=productdesc,price=price,crop_year=year,crop_quantity=quantity)
        db.session.add(products)
        db.session.commit()
        flash("Product Added","info")
        return redirect('/cropsproduct')
...
def register():
    ...
    if request.method=="POST":
        ...
        address=request.form.get('address')
        # farmingtype=request.form.get('farmingtype')     
        query=Register(farmername=farmername,city=city,phonenumber=phonenumber,address=address)
